...
export function HomePageClient() {
  ...
  // Redirect to login if not authenticated (client-side for SEO)
  useEffect(() => {
    if (!loading && !user) {
  ...
  const hasCompleteProfile = isGuestUser || !!(user?.gender && user?.city);
  ...
  const renderContent = () => {
    ...
    return <WebsiteIntroPage />;
  };

  return (
    <Flex style={{ height: "100dvh", overflow: "hidden", background: "var(--bg-primary)" }}>
      {/* ===== DESKTOP SIDEBAR ===== */}
      <nav aria-label="Điều hướng chính" style={{ display: "contents" }}>
      <Flex
        direction="column"
      ...
      {/* ===== MAIN CONTENT ===== */}
      <main id="main-content" style={{ flex: 1, minWidth: 0, height: "100%", display: "flex", flexDirection: "column" }}>
        {/* Navbar */}
        <Flex
          align="center"
          justify="between"
      ...
      {/* ===== MOBILE BOTTOM NAV ===== */}
      {hasCompleteProfile && (
      <nav aria-label="Điều hướng di động" style={{ display: "contents" }}>
      <Flex
