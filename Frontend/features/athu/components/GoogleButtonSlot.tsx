...
export function GoogleButtonSlot() {
  ...
  useEffect(() => {
    const t = window.setTimeout(() => setIsStale(true), 5000);
    return () => clearTimeout(t);
  }, []);

  // Show custom styled button when GIS failed or not available after timeout
  const needsFallback = isStale && !googleReady;
  ...
  if (needsFallback) {
    return (
      <button
        ...
        onMouseEnter={(e) => {
          e.currentTarget.style.borderColor = "var(--auth-muted)";
          e.currentTarget.style.boxShadow = "0 2px 8px var(--auth-line)";
        }}
        onMouseLeave={(e) => {
      ...
      >
        ...
        <Text style={{ color: "var(--auth-text)" }}>Tiếp tục với Google</Text>
      </button>
    );
  }
  ...
  return (
    <Box style={{ minHeight: 50, width: "100%" }}>
      ...
      {/* Google button (hidden until ready) */}
      <Box
        ref={buttonRef}
        style={{
          minHeight: 48,
